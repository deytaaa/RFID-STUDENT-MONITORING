...
void checkSerialCommands() {
  if (Serial.available()) {
    ...
    if (command.startsWith("ACCESS_GRANTED:")) {
      ...
      String cardID;
      int delaySec = 10; // Default to 10 seconds
      if (secondColon > 0) {
        cardID = command.substring(firstColon + 1, secondColon);
        delaySec = command.substring(secondColon + 1).toInt();
      } else {
        cardID = command.substring(15);
      }
      grantAccess(cardID, delaySec);
    } 
...
// ----------------------------
void grantAccess(String cardID, int delaySec) {
  ...
  // Wait for delaySec seconds before closing
  delay(delaySec * 1000);

  // ✅ Close gate
  gateServo.write(0);
  Serial.println("🚪 Gate CLOSED");
  sendBackendData("GATE_CLOSED", cardID, "Servo gate closed after delay");
